...
class RequestsKeywords(object):
    ...
    def put_request2(self, alias, uri, data={}, headers=None, files={}, allow_redirects=None):
        """ Send a PUT request on the session object found using the
        given `alias`
        ...
        data = self._utf8_urlencode(data)
        redir = True if allow_redirects is None else allow_redirects
        response = self._put_request2(session, uri, data, headers, files, redir)
    ...
    def _put_request2(self, session, uri, data, headers, files, allow_redirects):
        resp = session.put(self._get_url(session, uri),
                            data=data, headers=headers,
                            files=files,
                            cookies=self.cookies, timeout=self.timeout,
        ...
        # store the last response object
        session.last_resp = resp
        self.builtin.log("Put response: " + resp.content, 'DEBUG')
        return resp
